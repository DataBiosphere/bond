...
def default_config():
    conf = ConfigFactory.parse_file('bond.conf')
    return AuthenticationConfig(conf.get('authentication.BOND_ACCEPTED_AUDIENCE_PREFIXES', []),
                                conf.get('authentication.BOND_ACCEPTED_EMAIL_SUFFIXES', []),
                                conf.get('authentication.BOND_MAX_TOKEN_LIFE', 600))
...
class Authentication:
    ...
    # def require_user_info(self, token):
    #     if token:
    #         return True
    #     else:
    #         return False

    def require_user_info(self, request_state):
        ...
        if len(auth_header_parts) != 2 or auth_header_parts[0].lower() != 'bearer':
            abort(401, message='Malformed Authorization header, must be in the form of "bearer [token]".')
