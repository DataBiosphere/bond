...
docker run -v $PWD/app.yaml:/app/app.yaml \
    ...
    --entrypoint "/bin/bash" \
    ${BOND_IMAGE} \
    -c "gcloud auth activate-service-account --key-file=deploy_account.json && gcloud config set storage/thread_count 1 && gcloud config set storage/process_count 1 && gcloud config get storage/thread_count && gcloud config get storage/process_count && gsutil config -o -"
    # -c "gcloud auth activate-service-account --key-file=deploy_account.json && gcloud config set storage/thread_count 1 && gcloud config set storage/process_count 1 && gcloud app deploy app.yaml --project=$GOOGLE_PROJECT --verbosity=debug && gcloud app deploy cron.yaml --project=$GOOGLE_PROJECT --verbosity=debug"
