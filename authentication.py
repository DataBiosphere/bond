...
_ACCEPTED_AUDIENCE_PREFIXES = os.environ['BOND_ACCEPTED_AUDIENCE_PREFIXES'].split()
_ACCEPTED_EMAIL_SUFFIXES = os.environ['BOND_ACCEPTED_EMAIL_SUFFIXES'].split()
...
def require_user_info(request_state, token_info_fn=_token_info):
    ...
    :param token_info_fn: function to get token info, defaults to google http request, override for testing
    ...
    if user_info is None:
        logging.debug("auth token cache miss for token %s", token)
        user_info = _fetch_user_info(token, token_info_fn)
        # cache for 10 minutes or until token expires
        memcache.add(key=token, value=user_info, time=min([user_info.expires_in, 600]))

    return user_info


def _fetch_user_info(token, token_info_fn):
    ...
    if any(audience.startswith(prefix) for prefix in _ACCEPTED_AUDIENCE_PREFIXES) or \
            any(token_info.get('email').endswith(suffix) for suffix in _ACCEPTED_EMAIL_SUFFIXES):
